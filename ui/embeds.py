...
from resources.missing import MISSING
...
class Queuer[T](Embedder):
    ...
    def get_current(self, index: int) -> discord.Embed:
        ...
        receipt = discord.Embed(title=self.title, colour=self.colour)

        for element in considering:
            receipt.add_field(
                name=element.__getattribute__(self.__name), value=element.__getattribute__(self.__value), inline=False
            )
...
class Paginator[T](Embedder):
    ...
    def get_current(self, index: int) -> discord.Embed:
        ...
        considering = self.elements[(index - 1) * 13 : index * 13]
        # NOTE: slices won't rise an error even if the latter value exceeds list's length
        page = discord.Embed(title=self.title, description=self.description, colour=self.colour)

        for element in considering:
            match (self.__name, self.__value):
                case (str(name), str(value)):
                    page.add_field(name=getattr(element, name), value=getattr(element, value))
                case (int(f), int(s)):
                    page.add_field(name=element[f], value=element[s])  # type: ignore
                case _:
                    raise ValueError
