...
class YouTubeRequest:
    ...
    def get(id: str) -> list[tuple[str, str, str, str, str]]:
        with Connection() as cursor:
            query = 'select song_title, external_album_ids.youtube_id, artist_name, songs.duration, artists_ids.youtube_id as yt_art_id '
            joins = 'from songs inner join external_ids on external_ids.song_id = songs.rowid inner join external_album_ids on external_album_ids.album_id = songs.album_id inner join song_authors on song_authors.song_id = songs.rowid inner join artists_ids on song_authors.artist_id = artists_ids.rowid '
            condition = 'where external_ids.youtube_id = ?;'
            cursor.execute(query + joins + condition, (id,))
    ...
    async def _update_song(cursor: aiosqlite.Cursor,id: str, title: str, album_id: int, duration: str) -> None:
        ...
        if (song_id := await cursor.fetchone()) is not None:
            query = (
                ...
                'where not exists youtube_id and song_id = ?;'
            )
            params = (id, song_id[0])
        else:
            query = (
