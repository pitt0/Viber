...
class MusicPlayer:
    ...
    @classmethod
    def load(
        cls, guild: discord.Guild, voice_client: discord.VoiceClient
    ) -> "MusicPlayer":
        self = cls(guild)
    ...
    @classmethod
    async def create(
        cls, guild: discord.Guild, voice_channel: discord.VoiceChannel
    ) -> "MusicPlayer":
        self = cls(guild)
    ...
    async def voice_update(
        ...
        if self.playing:
            ...
            if len(after.channel.members) == 1 or all(member.bot or member.voice.deaf for member in after.channel.members):  # type: ignore
                ...
                print(
                    "Paused. Cause: Members in a voice channel were either all deafened or all bot."
                )
    ...
    async def play(self) -> None:
        ...
        while not self.queue.done():
            ...
            source = await self.get_source(SongsAPI.get_source(self.queue.current[0]))
            ...
            if not self.player.message:
                self.player.message = await self.channel.send(
                    embed=self.embed, view=self.player
                )
            else:
